...
| ------------ | ---------------------------------------------------------------------------------------------------------------- |
| Type         | ID prefixに従いbusiness requirement／constraint／gate／functional／data／non-functional                          |
...
| Source       | 個別表のSource。family表の場合は`30`の個別定義と`24`のSource mapping                                             |
...
| ID     | 要求                                                                                                                                         | Priority | Source                     | Verification                              |
| ------ | -------------------------------------------------------------------------------------------------------------------------------------------- | -------: | -------------------------- | ----------------------------------------- |
...
- v1 tag migration、structural／semantic audit、tag public projection
...
| ID          | 要件                                                                                                                                                                                                                                                                                                                                                                                                                                      | Priority | Source                   | Verification           |
| ----------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------: | ------------------------ | ---------------------- |
...
| Family | Count | Scope                                                                                                                               | Specifications                                       | Acceptance    |
| ------ | ----: | ----------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------- | ------------- |
...
| NFR-SEC-002 | public配信にCSP、X-Content-Type-Options、Referrer-Policy、frame制御を設定し、任意HTMLを描画しない。                              | header test               |
...
| SWEBOK V4.0a Software Requirements KA       | 本文書                                                  |
| ------------------------------------------- | ------------------------------------------------------- |
| 1.2〜1.7 要件分類                           | 業務要求、機能要件、非機能要件、プロジェクト制約を分離  |
| 2 Requirements Elicitation                  | 根拠資料、ペルソナ、既存実装、公式API・Policyを情報源化 |
| 3.1 Basic Requirements Analysis             | ID、単一判断、測定値、正常／異常／境界を定義            |
| 3.2 Economics of QoS                        | KPI-007、NFR-COST、予算停止順序                         |
| 3.4 Conflict                                | wordcloud価値とDeveloper Policiesの衝突をGATE-001で管理 |
| 4.3 Acceptance Criteria-Based Specification | 13.2から`23`のGiven/When/Thenへ追跡                     |
| 4.5 Additional Attributes                   | ID、Priority、Source、Verification、Confidence          |
| 4.6 Comprehensive Specification             | フェーズ差分でなく全対象を一文書に統合                  |
| 5 Requirements Validation                   | Fableレビュー、fixture試験、E2E、運用演習               |
| 6 Requirements Management                   | 2.3のGit変更管理、影響分析、承認                        |
...
| Q-009 | 外部channelの精査済み684動画を検索scopeへ含めるか              | 母集団、権利、tag migration  | 含める。sourceKindで区別し、production公開はGATE-001/002合格後だけとする              | Product Owner | closed   |
