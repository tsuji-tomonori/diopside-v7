...
本文書はtarget productの要求を定義し、現行実装の完成状態を表さない。現行リポジトリにはReact／Vite frontend、collector、processor、tagging pipeline、static exporter、operator CLI、AWS CDKのローカル実装がある。一方、AWS上の配備・権限・package・保持・queue・operator setupには未解決gapがあり、production acceptanceも未実施である。要求上の未確定事項は16節、レビュー上の残件は[21.requirements-review-log.md](21.requirements-review-log.md)、実装gapは[25.gaps-and-decisions.md](25.gaps-and-decisions.md)で管理する。
...
| SRC-UC-INPUT     | received use-case Markdown（2026-08-01再確認）                                                                  | UC-01〜10の受領原典。repo版はUC-11／12と現行境界を意図的に拡張                          |
...
| SRC-IMPL         | `README.md`、`frontend/src/lib/{schemas,contract}.ts`、`backend/src/app/runtime`、`infra/lib/diopside-stack.ts` | 現行frontend／runtime／CDK、current public contract、implementation gap                 |
...
- 動画一覧、検索、保存、履歴、動画詳細
- 個人運用者がIAM／STSでAdminRoleを引き受けて使用する管理CLIと静的運用レポート
...
- 常時稼働のEC2／ECS、またはDEC-DB-001の再検討条件を満たさないRDB／OpenSearch
...
| CON-010 | ローカル実装の存在、固定fixtureでの検証、AWSへのdeploy、production acceptanceを別の状態として記録し、いずれかを他の完了証拠にしてはならない。                                                         | confirmed  | AC-DEV-02              |
...
現行CDK／runtimeのresource、権限境界、未実装経路を含むdeployment viewは[architecture.drawio.svg](../design/architecture.drawio.svg)を正とする。次図は要求上の責務と主要flowを簡略化したものであり、deploy済み状態を示さない。

```mermaid
flowchart TB
  Viewer[視聴者] --> CDN[CloudFront]
  CDN --> Public[(S3 public data)]
  Web[React / Vite build] -. asset配備未実装 .-> CDN

  Operator[個人開発者] --> CLI[IAM / STS operator CLI]
  Scheduler[EventBridge] --> Enqueue[Collector / Exporter enqueue]
  CLI --> Queue[SQS job / export queue]
  Enqueue --> Queue

  Queue --> Workers[Processor / Exporter Lambda]
  YT[YouTube公式API] --> Workers
  Workers --> Stores[(S3 raw / processed / config / public)]
  Workers --> State[(DynamoDB control)]
  Queue --> DLQ[DLQ / CloudWatch / SNS]
```
...
現行targetでは、個人運用者だけがIAM／STSでAdminRoleを引き受けたCLIを使用し、AWS SDK経由でジョブ一覧、ジョブ詳細、起動、キャンセル可能状態の停止、DLQ一覧、redrive、quota／費用概要、GATE evidence list／create／replace、deletion request create／status／retryを行う。独自HTTP API、管理Web UI、cookie session、CSRF flowは初回targetの対象外とする。`SPEC-API-ADMIN-*`のIDとHTTP形式のpathは論理操作contract／将来adapterのmappingであり、現行CDKにAPI Gatewayや管理endpointが存在することを意味しない。必須成果はST-05以外のアクセス遮断、最小権限、service側throttlingへの対応、idempotency、全状態変更の監査である。詳細はSPEC-API-ADMIN-COMP-001とSPEC-OPS-DELETION-001を正とする。
...
| NFR-SEC-003 | 管理状態変更はIAM／STSでAdminRoleを引き受けたCLIに限定し、最小権限、service側throttlingへの対応、credential保護、監査ログを必須とする。 | security integration test |
...
6. VPC／NAT Gateway、OpenSearch、常時稼働workerを使わない。relational storeは現行アクセスパターンで必要になるまで追加しない（DEC-DB-001）。
...
GATE-003の証跡は、ap-northeast-1の当日単価、USD/JPY、税の扱い、無料枠を含む場合と含まない場合、CloudWatch Logs、Secrets Manager、EventBridge、SQS、Lambda、DynamoDB、S3、CloudFront、AWS Budgets等、そのreleaseで実際に採用する全serviceを明記した計算表とする。API Gatewayや新data storeを将来追加する場合は、採用前の変更影響と費用を同じ証跡へ追加する。standardは無料枠なしでも目標500円以下を目指し、上限1,000円以下を必須とする。peakは停止制御適用後に上限1,000円を超えないことをsimulationで証明する。
